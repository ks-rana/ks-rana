...
- [Do Large Language Models Know When They Are Wrong?](https://do-models-know-when-theyre-wrong.vercel.app) : original research. 89 expert-level questions × 3 prompts × 2 models — 534 responses, run with a research partner. Claude Opus 4.8 and GPT-5.6 scored about the same, then missed their own accuracy in opposite directions: one underconfident, one overconfident. Telling them to be humble shifted stated confidence significantly (*p* = .004) and made calibration worse for both. The report runs the study's instrument on the reader before it tells them anything, and puts the ceiling effect that limits every claim on the page rather than in a footnote.
